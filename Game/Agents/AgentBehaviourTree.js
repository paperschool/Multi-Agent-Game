...
class Agent_Behaviour {
  ...
  constructor(agentObject) {
    ...
    */
    ...
    // - attempt to move towards the focust position
    ...
    // this task will update the agents understanding of the player position
    BehaviourTree.register('setAgentFocusPosition', new BehaviourTree.Task({
      title:'setLastKnownPosition',
      run:function(agent){
        agent.setAgentFocusPosition();
        this.success();
      }
    }));
    ...
    BehaviourTree.register('playerWithinLineOfSight', new BehaviourTree.Task({
      ...
      run:function(agent){
        agent.playerLineOfSight() ? this.success() : this.fail();
      }
    }));
